...
func _post_import(scene : Node):	
	print("=== POST IMPORT TRIGGERED ===")
	print("Scene name: ", scene.name)
	print("Scene children count: ", scene.get_child_count())
	for i in range(scene.get_child_count()):
		print("Child ", i, ": ", scene.get_child(i).name, " (", scene.get_child(i).get_class(), ")")
	...
	if object_prefix == "SM":
		...
		if children.is_empty():
			...
			if (children.size() == 1 and children[0] is StaticBody3D):
				print("Collision was already generated by Godot")
			else:
				var i = 0
